...
def evaluate_seo_research_quality(
    program_path: str,
    results_dir: str,
    test_niches: List[str] = ["digital marketing", "fitness equipment", "online courses"],
    ...
    try:
        ...
        for result in research_results:
            ...
            keyword_diversity_score = min(len(strategy["primary_keywords"]) / 15.0, 1.0)  # Harder: Max 15 keywords for full score
            competitor_analysis_score = min(len(strategy["competitor_domains"]) / 10.0, 1.0)  # Harder: Max 10 competitors
            strategy_depth_score = min(len(strategy["search_strategies"]) / 12.0, 1.0)  # Harder: Max 12 strategies
            ...
            for keyword in strategy["primary_keywords"]:
                if isinstance(keyword, str):  # Safety check
                    ...
                    if any(word in keyword_lower for word in ["buy", "price", "cost", "cheap", "best", "review", "compare"]):
                    ...
                    if any(word in keyword_lower for word in ["ai", "automated", "generator", "tool", "software"]):
            ...
            if use_tavily_search:
                try:
                    ...
                    search_count = 0
                    for keyword in strategy["primary_keywords"][:3]:  # Test first 3 keywords
        ...
        metrics = {
            ...
            "public": {
                ...
                "average_competitors_per_niche": sum(len(r["strategy"]["competitor_domains"]) for r in research_results) / len(research_results)
            },
            "private": {
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="SEO research evaluation script"
    )
    parser.add_argument(
        "--program_path",
        type=str,
